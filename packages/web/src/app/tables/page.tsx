...
type ManualTableChange = {
  label: string;
  value: string;
};

function formatManualValue(value: string) {
  return value === '' ? '(공백)' : value;
}

function collectChangedManualValues(currentValue: unknown, otherValue: unknown): ManualTableChange[] {
  ...
  const values: ManualTableChange[] = [];
  ...
  for (let tableIndex = 0; tableIndex < tableCount; tableIndex += 1) {
    ...
    const currentHeaderCount = Array.isArray(currentTable?.headers) ? currentTable.headers.length : 0;
    const otherHeaderCount = Array.isArray(otherTable?.headers) ? otherTable.headers.length : 0;
    const headerCount = Math.max(currentHeaderCount, otherHeaderCount);
    ...
    for (let headerIndex = 0; headerIndex < headerCount; headerIndex += 1) {
      ...
      if (currentHeader !== otherHeader) {
        values.push({
          ...
          value: formatManualValue(currentHeader),
        });
      }
    }
      ...
      for (let cellIndex = 0; cellIndex < cellCount; cellIndex += 1) {
        ...
        if (currentCell !== otherCell) {
          values.push({
            ...
            value: formatManualValue(currentCell),
          });
        }
      }
    }
...
function renderManualTableChanges(currentValue: unknown, otherValue: unknown, side: 'before' | 'after') {
  const values = collectChangedManualValues(currentValue, otherValue);
  if (values.length === 0) {
    return <span className="tables-log-value-empty">없음</span>;
  }
    ...
    <div className="tables-log-multi-value">
      {values.map((entry, index) => (
        <div className="tables-log-change-entry" key={`${side}-${index}-${entry.label}-${entry.value}`}>
          ...
          <span className="tables-log-diff-inline">{entry.value}</span>
        </div>
      ))}
    </div>
  );
...
function buildLogSections(log: TableEditLog): LogSection[] {
  ...
  if (log.entityType === 'column_meta') {
    ...
    if (JSON.stringify(before.manualTables) !== JSON.stringify(after.manualTables)) {
      sections.push({
        label: '참고표',
        before: renderManualTableChanges(before.manualTables, after.manualTables, 'before'),
        after: renderManualTableChanges(after.manualTables, before.manualTables, 'after'),
      });
    }
